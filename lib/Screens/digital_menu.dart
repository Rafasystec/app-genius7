...
//    const Choice(2,title: 'Log out', icon: Icons.exit_to_app),
      ...
      body:  ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          return Column(
            ...
            children: <Widget>[
              Text(item.description,style: TextStyle(fontSize: 20.0,fontStyle: FontStyle.italic),),
              Container(
                height: 250,
                child: listCategoryItem(item),
              ),
  ...
  Widget listCategoryItem(Category category){
    return ScrollParent(
      child: ListView.builder(
          itemCount: category.itens.length,
          itemBuilder: (BuildContext context,int index){
            var item = category.itens[index];
            return GestureDetector(
              onTap: () async{
                Navigator.of(context).push(MaterialPageRoute(builder: (context) => ScreenDigitalMenuItem(item)));
                //Fluttertoast.showToast(msg: 'Click');
              },
              child: getItemDetail(item),
            );
          }
      ),
      controller: _controller,
